...
class Submission:
    ...
    def parseOutAuthors(self):
        ...
        while (i):
            ...
            affiliation = convertUtf8ToHtml(self.rawFields["Primary Affiliation %d - Institution" % i])
            affiliationCountry = abbreviate_country(self.rawFields["Primary Affiliation %d - Country" % i])
            ...
            if (lastName):
                if (rawAuthors and
                    ...
                    rawAuthors[-1][3].append(Affiliation(affiliation, affiliationCountry))
                else:
                    # First appearance of author for this submission
        	        rawAuthors.append([firstName, middleName, lastName, [Affiliation(affiliation, affiliationCountry)], email])
            else:
                if (affiliation):
...
class Session:
    ...
    def __init__(self, rawFields, chiProgram, utf8ToHtml=False):
        ...
        self.hasHonorableMention = False
        self.hasAward = False
    ...
    def parseData(self):
        ...
        for subId in self.rawFields["Submission IDs"].split():
            if (subId in self.chiProgram.submissions):
                ...
                self.hasHonorableMention = self.chiProgram.submissions[subId].bestPaperNominee or self.hasHonorableMention
                self.hasAward = self.chiProgram.submissions[subId].bestPaperAward or self.hasAward
            else:
        ...
        for submission in self.submissions.itervalues():
            ...
            for author in subAuthors:
                ...
                for affiliation in author.affiliations:
                    affiliationHash = affiliation.name
    ...
    def addExtraAuthorsFromDummySubmissions(self):
        ...
        for submission in self.submissions.itervalues():
            if ("combine" in submission.notes):
                ...
                for author in dummySubmission.authors:
                    author.submissions.remove(dummySubmission)
                    author.submissions.append(submission)
